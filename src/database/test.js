...
Database.then(async (db) => {
    ...
    await saveOrphanage(db, { 
        lat: "-27.222633",
        lng: "-49.6555874",
        name: "Lugar de Gratidão",
        about: "Presta assistência a crança de 06 a 15 anos que se encontre em situação de risco e/ou  vulnerabilidade social.",
        ...
        oppening_hours: "Horário de visita das 18 até as 20h",
        open_on_weekends: "1"
    ...
    // //consultar somente 1 orphanato, pelo idx
    const orphanage  = await db.all('SELECT * FROM orphanages WHERE id = "4"')
    console.log(orphanage)

    // //deletar dado da tabela -- await db.run("DELETE FROM orphanages") para deletar toda tabela --
